...
namespace Tests
{
    ...
    public class TelemetryTests
    {
        ...
        public void EventSent()
        {
            ...
            AssertTelemetryReceived(new Dictionary<string, int> {
                                        { Telemetry.EventsName, 1 } });
        }

        [Test]
        public void PacketSent()
        {
            _telemetry.PacketSent(42);
            AssertTelemetryReceived(new Dictionary<string, int> {
                                        { Telemetry.PacketsSentName, 1 },
                                        { Telemetry.BytesSentName, 42 } });
        }

        [Test]
        public void PacketDropped()
        {
            _telemetry.PacketDropped(42);
            AssertTelemetryReceived(new Dictionary<string, int> {
                                        { Telemetry.PacketsDroppedName, 1 },
                                        { Telemetry.BytesDroppedName, 42 } });
        }

        [Test]
        public void PacketsDroppedQueue()
        {
            _telemetry.PacketsDroppedQueue();
            AssertTelemetryReceived(new Dictionary<string, int> {
                                        { Telemetry.PacketsDroppedQueueName, 1 } });
        }
